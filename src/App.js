...
function App() {
    ...
    const onSubmit = (e,user) => {
        ...
        setTimeout(() => {
            setLoading(false);
            //targetId -> if true check password, if false return error
            //targetPassword -> if true login succeed, if false return error.
            const targetId = user.userData.find(user => user.id === id)[0].password;
            if (password === "aşlflşidşflgkdfişglkdişflgk"){
